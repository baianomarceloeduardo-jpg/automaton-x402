 ...
 *
 * Config via env:
 *   VALUE_API_BASE      base URL of the API (default http://127.0.0.1:8080)
 *   VALUE_API_PAYMENT   default X-PAYMENT tx hash to attach to paid calls (optional)
 *
 * Usage (MCP client config):
 *   { "mcpServers": { "automaton-value-api": { "command": "node",
 *       "args": ["C:\\\\root\\\\value-api\\\\mcp-server.js"],
 *       "env": { "VALUE_API_BASE": "https://<id>.lhr.life" } } } }
 */
const http = require('http');
const https = require('https');
const { URL } = require('url');

const BASE = (process.env.VALUE_API_BASE || 'http://127.0.0.1:8080').replace(/\/+$/, '');
  ...
  while ((nl = buf.indexOf('\n')) >= 0) {
    ...
    if (msg.method === 'initialize') {
      reply(msg.id, { protocolVersion: '2024-11-05', capabilities: { tools: {} },
        serverInfo: { name: 'automaton-value-api', version: '1.0.0' } });
